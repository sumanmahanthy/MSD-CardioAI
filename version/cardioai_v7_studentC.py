# ================================================================
# CARDIOAI V7 — STUDENT C: DenseNet169 (Dense-Skip Specialist)
# Role in ensemble: Dense skip-connection bias, different from B7/ViT
# Architecture diversity: DenseNet skip patterns vs EfficientNet/ViT
# Same fixes as Student-A/B: warmup, soup, TTA export, correct ordering
# Output: preds_C_val.npy, preds_C_test.npy
# ================================================================
...
BATCH_SIZE      = 48 * max(1, num_gpus)   # DenseNet169 lighter → bigger batch OK
EPOCHS          = 15
WARMUP_EPOCHS   = 3
LR_BACKBONE     = 4e-5
LR_HEAD         = 1e-4
...
print(f"STUDENT-{MODEL_TAG} (DenseNet169) | {DEVICE} | GPUs:{num_gpus} | Batch:{BATCH_SIZE}")
...
# ────────────────────────────────────────────────────────────────
# 3. MODEL — DenseNet169 + deep head
# ────────────────────────────────────────────────────────────────
class DenseNetStudent(nn.Module):
    def __init__(self):
        super().__init__()
        self.backbone = timm.create_model(
            "densenet169", pretrained=True, num_classes=0, drop_rate=0.2
        )
...
model = DenseNetStudent().to(DEVICE)
if num_gpus > 1:
